...
function SearchDialog({ open, onOpenChange }: SearchDialogProps) {
  ...
  const [sources, setSources] = useState<string[]>(["kijiji", "bamboo", "craigslist"]);
        ...
        {open ? (
          <DialogPrimitive.Portal forceMount>
            ...
            <DialogPrimitive.Content asChild forceMount>
              <motion.div
                className="fixed left-1/2 top-1/2 z-50 w-[min(520px,calc(100vw-32px))] -translate-x-1/2 -translate-y-1/2 rounded-[28px] bg-card p-6 shadow-button"
              ...
              >
                ...
                <form onSubmit={onSubmit} className="flex flex-col gap-4">
                  <Field label="City">
                    <Input
                      placeholder="Waterloo (defaults to your config)"
                      value={city}
                  ...
                  <div className="flex items-center justify-between rounded-2xl bg-background px-3 py-2.5">
                    <div className="flex flex-col">
                      ...
                      <Typography variant="caption" className="text-muted-foreground">
                        Enrich & rank with Claude (needs ANTHROPIC_API_KEY)
                      </Typography>
                    </div>
